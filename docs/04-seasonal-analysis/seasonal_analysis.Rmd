...
The seasonal plot splits the series by its seasonal cycle (year) and compare the change between each cycle (or from year to year). In the following example we will create a seasonal plot with plotly:
...
**Note:** that we use a color scale to mark the years. This enables us to quickly observe the growth in the series from year to year, and the seasonal patter. 

Similarly, we can split the series by it's frequency units and plot each frequency unit against the frequency cycle (e.g., year):
...
Heatmap is another visualization tool for review and identify seasonal pattern in a series:
...
Likewise the box-plot seasonal plot, we can use the kernel density plot to visualize the series frequency units distribution. A simple method is to use the **ggplot2** package `geom_density` function:
...
As before, the detreded series should have narrow distribution:
...
#### feasts Seasonal Plots

The **feasts** package provides some built-in functions for seasonal plots of `tsibble` objects based on the **ggplot2** package. The `gg_season` function create a seasonal plot by the series frequency cycles units. By default, the function label each line by its frequency cycle unit:
