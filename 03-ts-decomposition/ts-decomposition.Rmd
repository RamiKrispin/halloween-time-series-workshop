...
The STL (Seasonal and Trend decomposition using LOESS) method is an advanced decomosition approach which is based on the LOESS smoothing method (locally estimated scatterplot smoothing). The main advantage of the STL method (vs. the classical method) is the ability to control the smoothing period of both the trend and seasonal component. That become super useful series has a multiplicative seasonality. The STL main parameters:

* Seasonal window - defines the number of years to use for calculate the seasonal component. A smalest number of years will enable to capture changes of the seasonal component over time (multiplicative seasonality). 
...
In R, the `stl` function from the **stats** package is the main function for STL deecomposition of `ts` objects. The `STL` function from the **feasts** package provides a wraper for the `stl` function for `tsibble` objects.
...
You can notice from the trend ans seasonal component estimation above:
...
As we will keep reducing the season window the better the STL model will capture the multiplicative seasonal of the series. It is recommanded not to reduce the window below to 5:
